...
frappe.ui.form.on("Moldova Bank Statement Import", {
	...
	import_file(frm) {
		...
		frappe.run_serially([
			// Convert DBO to CSV if .txt file
			() => {
				if (frm.doc.import_file && frm.doc.import_file.toLowerCase().endsWith(".txt")) {
					return frm
						.call({
							method: "convert_dbo_to_csv",
							args: {
								data_import: frm.doc.name,
								dbo_file_path: frm.doc.import_file,
							},
						})
						.then((r) => {
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/erpnext_moldova_banking.moldova_banking.doctype.moldova_bank_statement_import.moldova_bank_statement_import.download_errored_template",
			{
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/erpnext_moldova_banking.moldova_banking.doctype.moldova_bank_statement_import.moldova_bank_statement_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		frappe.call({
			method: "erpnext_moldova_banking.moldova_banking.doctype.moldova_bank_statement_import.moldova_bank_statement_import.get_import_logs",
			args: {
				docname: frm.doc.name,
			},
			callback: function (r) {
